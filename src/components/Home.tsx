...
const Home = ({ smootherRef }: HomeProps) => {
	...
	useGSAP(() => {
		...
		let splitfirst, splitname, splitsexond;
		const setupSplitText = () => {
			// Only run this DOM-heavy operation once the page is interactive
			splitfirst = SplitText.create(".firstline", { type: "chars" });
			splitname = SplitText.create(".name", { type: "chars" });
			splitsexond = SplitText.create(".secondline", { type: "words" });

			// Now run the animation timeline after setup is complete
			runAnimations();
		};
		...
		const loadTimeout = setTimeout(setupSplitText, 500);
		...
		return () => {
			...
			if (splitsexond) splitsexond.revert();
			// Cleanup on unmount
